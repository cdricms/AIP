...
if version.major == 3 and version.minor >= 7:
    ...
    settings_res = requests.get(
        "https://gist.githubusercontent.com/Smoqu/704d343e972375a215eb92b04ca75a5f/raw/50cc4ffd7e984bccf379e5f0fa5f1eb008704b34/settings_template.json")
    ...
    shutil.move(f"./{folder_name}/commands", "./")
    ...
    os.system("touch .env")
    os.system(
        "echo TOKEN=GitHub token (must have repo and delete_repo enabled) >> .env")

    with open("settings.json", "w") as write_file:
        settings = json.dumps(settings_data, indent=2)
        write_file.write(settings)

    aipconfig_version = {"version": aip_version}

    with open('aipconfig.json', 'w') as write_file:
        conf = json.dumps(aipconfig_version, indent=2)
        write_file.write(conf)
